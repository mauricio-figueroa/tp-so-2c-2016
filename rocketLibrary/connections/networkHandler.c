 ...
 */
int librocket_enviarMensaje(int sockfd, const void * datos, size_t bytesAenviar, t_log* t_log) {
	...
	if (bytes_enviados == -1) { //ERROR
		...
		librocket_enviarMensaje(sockfd,datos+bytes_enviados,nuevosBytes, t_log);
	}else if(bytes_enviados == bytesAenviar){
		//mandado piola
 ...
 */
int librocket_recibirMensaje(int sockfd, void* buffer, size_t bytesAleer, t_log* log) {
	...
	if (bytesHeader == -1) { //ERROR
		...
		char* mensaje = malloc(200);
		sprintf(mensaje,
				"Se recibieron %d bytes de %d esperados en la linea %d del archivo %d",
		...
		librocket_recibirMensaje(sockfd,buffer+bytesHeader,nuevosBytes, log);
	}else if(bytesHeader == bytesAleer){
		//se envio ok!!
		return 1;
	}
				...
				if (i == listener) {
					...
					if ((nbytes = recv(i, buf, sizeof buf, 0)) <= 0) {
						...
						if (nbytes == 0) {
							// connection closed
							char* msj = malloc(100);
							sprintf(msj, "Se desconecto el socket %d", i);
							log_info(infoLogger, msj);
							free(msj);
						...
						close(i); // desconecto
						FD_CLR(i, &master); // remuevo de la lista de conexiones
					} else {
						// se recibe informacion de un cliente
						//hacer algo con la informacion recibida
						funcionParaMensajeRecibido(i);
					}

				} //end for que recorre todos los file descriptors
				*/
			}
...
void * hiloManejador(void* sockfd) {
	...
	t_log* logger =  log_create(LOG_INFO_PATH,LIBRARY_NAME,false,LOG_LEVEL_INFO);
